...
from PySide6.QtWidgets import  QWidget, QPushButton, QLabel, QHBoxLayout, QStackedWidget
...
class OnboardSettings(QWidget):
    ...
    def update_checkboxes(self):
        ...
        # Temporarily disconnect signals to avoid emitting during programmatic changes
        self.start_up_checkbox.stateChanged.disconnect(self.start_up_status)
        self.idle_time_checkbox.stateChanged.disconnect(self.idle_time_status)

        # Set checkbox states based on stored settings
...
class RequestThread(QThread):
    ...
    def run(self):
        try:
            response = requests.get(self.url, headers={"Authorization": f"Bearer {self.token}"}, params=self.params)
            if response.status_code == 200:
                print("Request successful")
            else:
                print(f"Request failed: {response.status_code}, {response.text}")
